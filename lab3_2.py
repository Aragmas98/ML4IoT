...
interpreter = tf.lite.interpreter(model_path='./models/{}.tflite'.fomat(args.model))
...
audio = pyaudio.PyAudio() #create a pyaudio instantiation 
stream = Audio.open(format=resolution, rate=samp_rate, channels=1,
                    input_device_index=dev_index, input=True, 
                    frames_per_buffer=chunk)
stream.stop_stream() 

COMMANDS = ['down', 'go', 'left', 'no', 'right', 'stop', 'up', 'yes'] #['stop', 'up', 'yes', 'right', 'left', 'no', 'silence', 'down', 'go']
...
while True:
    ...
    stft = tf.signal.stft(sample, length, stride, fft_length=length) 
